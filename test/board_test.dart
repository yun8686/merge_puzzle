...
void main() {
  ...
  group('2体を通るお手本', () {
    test('離れた2体を1本で通る道を返す', () {
      ...
      final hint = board.findPairHint();

      expect(hint, contains(const Cell(0, 0)));
      expect(hint, contains(const Cell(0, 4)));
    ...
    test('敵が1体しか居なければ空', () {
      ...
      expect(board.findPairHint(), isEmpty);
    });
  });

  group('敵を置く場所', () {
    test('決め打ちのマスに据えられ、残りは散る', () {
      final board = Board(rng: Random(3));
      board.buildStage(
        foes: const [
          FoeSpec(3, at: Cell(2, 2)),
          FoeSpec(4, at: Cell(6, 0)),
          FoeSpec(5),
        ],
      );

      expect(board.tileAt(const Cell(2, 2))!.ward, 3);
      expect(board.tileAt(const Cell(6, 0))!.ward, 4);
      expect(board.foeCells.length, 3);
      // 散らしたほうが決め打ちのマスに重ならない。
      final rest = board.foeCells
          .where((c) => c != const Cell(2, 2) && c != const Cell(6, 0));
      expect(rest.length, 1);
      expect(board.tileAt(rest.first)!.ward, 5);
    });
  });
}
