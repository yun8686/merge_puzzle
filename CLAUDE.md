...
`Spread` は「次の1本だけ、その相だけでつなげる」スキルだが、**色の決まりが
無くなったので何も変わらない**（README 第22段階）。`Board.spreadPhase` を立てて
1本編んだら下ろす配線と、`Board.hasSamePhaseRun` の空振り判定と、画面の
`_RuleNote` はそのまま残してある。**効き目が無いまま説明文だけ画面に出ている**
ので、決まりを入れ直すか、スキルを別のものに差し替えるかはまだ決めていない。
...
**つなぐときに色は見ない**（`Board.isConnected` / `canExtendPath`、README 第22
段階）。隣り合っていて、同じマスを二度通らず、3枚（`Board.minPathLength`）以上
あればチェインになる。**同じ色どうしでもつながる。**

**盤面に出る色は編成で決まる**（`Party.phases`）。色はもうチェインの長さを
縛らないが、**スキルの条件が読む**――雷の「3色を含む」、霜の「その色から
始めた」、焔・烈火の「その色を N 枚以上」。

**この決まりは、長さの上限を外した。** `python3 tools/sim/chain_length.py` の
`free` で測ると、敵マスを通る最長パスは**打ち切りの 12 枚に毎回届く**（色の
決まりがあった頃は平均 7.4 枚、8枚以上が 51%）。実際の上限は盤面の 48 マスで、
**威力＝つないだ枚数**なので、指を動かすだけでいくらでも威力が出る。難易度を
戻したくなったら、まず**チェインの長さに上限を入れる**か、**威力を枚数から
外す**こと（守りを上げるのは焼け石に水）。

**手詰まりで階層を落とすことは、事実上なくなった。** `Board.hasAnyChain` は
マスが3枚つながっていれば真になる。負けるのは体力が尽きたときだけ
（`GamePhase.floorLost` はほぼ通らない）。

**難易度は「ランダムに置いた敵1体のマスを通る最長パスの平均」で測る。**
`python3 tools/sim/chain_length.py` で出せる。「継げる隣マスの割合」では差が
出ない。**選べること自体が長さを押し上げる**ため。

**編成の相は2種類以上**（`Progress.minPhases`）。1色でもつなげるようになったが、
**盤面が単色だと色を読む意味がなくなる**（雷も霜も死ぬ）ので、編成の決まりとして
残してある。
...
**稽古は1本だけ。** 初めて遊ぶ人の「つなぎ方から反撃まで」を、初回に拠点の
上へ出す。3色で初めて潜る人に巡回の決まりを教える筋書きが別にあったが、
**教える決まりが無くなったので落とした**（README 第22段階）。
`Progress.taughtPrism` は保存の形を変えないために読み書きだけ残してある。
...
1. チェインをつなぐ … なぞってつなぐ。3枚で成立。**色は問わない**
2. 長いほど強い … 枚数＝威力。6枚
3. 防御を破る … マスの数字は越えるべき線。防御3を3枚で倒す
4. まとめて当てる … 2体を通る道。**2体とも同じ防御3・体力1で、両方倒せる**
5. 体力のある敵 … 体力2を1体だけ。5枚では防御を破っても削り切れない
6. 削り切る … **同じ敵にもう一度**。与えたダメージは残っている
7. 一撃で倒す … 防御を上回るほど大きく削れる。威力6なら体力2も1本で
8. 毎ターンの反撃 … 残った1体を倒し切る（ここで盤面が空になり終い）

5〜7 は同じ相手（防御5・体力2）の三段。2本で削り切ってから、**同じ敵を1本で
倒せる威力**を見せる。

**「届かないとき」「並びを変えて倒す」は落とした**（README 第22段階）。どちらも
「同じ色で塞げば鎖が行から出られない」という仕掛けの上に立っていて、色の決まりが
無くなると成立しない。
