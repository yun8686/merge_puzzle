...
**盤面に出る相は編成で決まる。** 継ぎ方の決まりは「隣り合う2枚は違う相。かつ、
どの N+1 枚を取っても N 色すべてが出る」。2相ならこれが交互と完全に同じ意味になる。
3相なら「同じ2色の往復は3枚まで」で、A→B→A は継げるがその次の B は継げない。

ここは両側から締まっている。**緩めすぎ**（「隣と違えばよい」）にすると3相でほぼ
全部の盤面が12枚編めるようになり、希少な相のジレンマも雷の8枚条件も意味を失う。
**締めすぎ**（「直前 N−1 枚と違う」）にすると3相で次に置ける相が常に1種類になり、
その相は盤面の 1/3 しか無いので目に見えて詰まる。**2色の交互を混ぜられるように
してもこれは直らない**（2色に絞っても次は1種類のまま）。README 第7・第8段階に
測った数字がある。
