...
**稽古場の敵は置く場所を決め打ちにする**（`FoeSpec.at`）。散らすと隣り合って
出ることがあり、最初の稽古で**敵と敵を繋いでいる**ように見える。3体とも4マス
以上離し、下の2体は1本の鎖で通せる間合いに置いてある。3体居るのは「守りを破る」
で1体討ったあとにも2体残すため――2体だけだと「まとめて当てる」が試しようが
なくなる。本番の階層では据えないこと。毎回同じ場所に同じ敵が出る。

**「1本の鎖は通った敵すべてに当たる」も稽古にする。** 判定は
`GameController.lastFoesHit`（直前の**1本**が傷をつけた敵の数。累計の
`felledWards` とは別）。お手本も専用で、`Board.findPairHint` が2体を通る道を
探す（`_Lesson.pairHint`）。普段の道を出すと、示した手では課題が進まない。
`findPathThrough` に `also`（このマスも通る）を足してあるが、**`cap` で伸ばす
上限を必ず押さえること**。長さが足りただけでは止まらなくなるので、上限が無いと
見つからない盤面で盤面いっぱいまで伸ばし続ける。残り1体になったら課題は畳む
（試しようがない）。
...
覚えることを1つずつ積み、**前の課題で作った盤面が次の課題の材料になる**
ように並べてある。

1. 鎖を編む … なぞって継ぐ／同じ相は続かない／3枚で成立
2. 長いほど強い … 枚数＝威力。まず6枚
3. 守りを破る … マスの数字は越えるべき線。守り3を討つ
4. まとめて当てる … 1本の鎖は通った敵すべてに当たる。2体を通す
5. 削って討つ … 1本で討てない敵。傷は残るので、もう一度当てる
6. 毎ターンの反撃 … 敵は毎手殴る。残りを討ち果たす

**4 が 5 を用意している。** 2体を通した鎖は厚いほうを討ち切れずに傷だけ
残すことが多く、その敵がそのまま 5 の教材になる。「傷は残る」を言葉では
なく盤面で見せられる。3 で1体討ったあとにも2体残るよう、敵は3体。

**試しようがなくなった課題は畳む。** 残り1体なら「まとめて当てる」を、
体力持ちの敵が居なくなったら「削って討つ」を、それぞれ done にする。
稽古場で詰ませない。
