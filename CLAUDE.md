...
**開いた瞬間のお手本だけは待たない。** 1手目は何をどうなぞるのかが分からない
ので、「手が止まってから6秒」（`_hintAfter`）を待たせると、その6秒はただ
止まっているだけになる。稽古場を開いた最初のフレームで `showHint` を呼ぶ
（盤面が組み上がってからでないと道が引けないので post-frame）。2手目からは
止まったときだけ。
